...
//服务层是无状态的，不存在并发问题，使用单例模式是从节约资源、简化调用接口、方便拓展等角度考虑的
...
// Login 用户登录，准备在此扩展为短信登录，暂时使用的是下面的密码登录方法
// 参数：登录请求信息
// 返回：成功消息、用户信息、错误信息
...
// 更新用户信息，先根据uuid查询到对应的用户，然后对请求中非空的字段进行判断并写入useinfo，最后把userinfo写入数据库
func (s *userInfoService) UpdateUserInfo(req *request.UpdateUserInfoRequest) (string, int) {
	var userInfo model.UserInfo
	if res := dao.GormDB.Where("uuid = ?", req.Uuid).First(&userInfo); res.Error != nil {
		zaplog.Error(res.Error.Error())
	...
	if res := dao.GormDB.Save(&userInfo); res.Error != nil {
		return "更新用户信息失败", -1
	}
	return "更新用户信息成功", 0
}
